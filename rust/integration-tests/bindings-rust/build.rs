...
use memorymap_compiler::ir::instance_names::{InstanceNames, calculate_instance_names};
...
fn main() {
    let dir = root_dir().join("test_memory_maps");
...
fn generate_type_ref_imports(ctx: &IrCtx, refs: &TypeReferences) -> TokenStream {
    ...
    if refs.use_bitvec {
        code.extend(quote! { use clash_bindings::bitvector::BitVector; });
    }
    if refs.use_index {
        code.extend(quote! { use clash_bindings::index::Index; });
    }
    if refs.use_mask {
        code.extend(quote! { use clash_bindings::mask::Mask; });
    }
    if refs.use_signed {
        code.extend(quote! { use clash_bindings::signed::Signed; });
    }
    if refs.use_unsigned {
        code.extend(quote! { use clash_bindings::unsigned::Unsigned; });
    }
