...
//!
//! Alternatively, one can make use of `clash_macros::Index!` and `clash_macros::index!` to make
//! types/instances with the correct parameters.
#![deny(missing_docs)]

use super::{FromAs, signed::Signed, unsigned::Unsigned};
...
pub trait IndexSizeCheck {
    ...
    /// ```
    /// # use clash_bindings::index::{Index, IndexSizeCheck};
    /// trait Marker {
    ///     fn nothing();
    /// }
    ///
    ...
    Index<N, T>: IndexSizeCheck<Inner = T>,
{
    ...
    #[inline]
    unsafe fn idx_new_unchecked(val: Self::Inner) -> Self {
        unsafe { Index::<N, T>::new_unchecked(val) }
    }
