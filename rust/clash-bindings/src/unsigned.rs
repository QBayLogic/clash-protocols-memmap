...
//!
//! Alternatively, one can make use of `clash_macros::Unsigned!` and `clash_macros::unsigned!`
//! to make types/instances with the correct parameters.
#![deny(missing_docs)]

use super::{FromAs, index::Index, signed::Signed};
...
pub trait UnsignedSizeCheck {
    ...
    /// ```
    /// # use clash_bindings::unsigned::{Unsigned, UnsignedSizeCheck};
    /// trait Marker {
    ///     fn nothing();
    /// }
    ///
