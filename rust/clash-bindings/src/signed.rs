...
//!
//! Alternatively, one can make use of `clash_macros::Signed!` and `clash_macros::signed!` to
//! make types/instances with the correct parameters.
#![deny(missing_docs)]

use super::{FromAs, index::Index, unsigned::Unsigned};
...
pub trait SignedSizeCheck {
    ...
    /// ```
    /// # use clash_bindings::signed::{Signed, SignedSizeCheck};
    /// trait Marker {
    ///     fn nothing();
    /// }
    ///
